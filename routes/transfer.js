...
// --- 1. NEW: Fetch Vehicle & Seller Details ---
    ...
    try {
        const sql = `
            SELECT 
                ...
                v.vehicle_class, v.make, v.model, v.engine_number, v.chassis_number,
...
// --- 2. NEW: Fetch Buyer Details by PAN ---
...
router.post('/fetch-buyer-details', async (req, res) => {
    ...
    try {
        const [rows] = await pool.query(
            'SELECT user_id, full_name, address_line1, city, state, pincode, phone_number FROM Users WHERE pan_number = ?',
            [pan_number]
        );
        if (rows.length === 0) {
...
router.post('/create-transfer-payment', async (req, res) => {
    ...
    const transferFee = 500.00; // Your demo fee
    ...
    try {
        ...
        const [paymentResult] = await connection.execute(
            `INSERT INTO Payments (application_id, payment_for, amount, payment_status)
             VALUES (?, 'Ownership Transfer', ?, 'Pending')`,
            [application_id, transferFee]
        );
