...
router.post('/register', async (req, res) => {
  try {
    const { 
      full_name, dob, email, pan_number, phone_number, 
      address_line1, city, state, pincode, password 
    } = req.body;

    if (!full_name || !dob || !email || !pan_number || !phone_number || 
        !address_line1 || !city || !state || !pincode || !password) {
      return res.status(400).json({ message: 'Please provide all required fields.' });
    }

    const [existingUser] = await pool.query(
      'SELECT * FROM Users WHERE email = ? OR pan_number = ? OR phone_number = ?',
      [email, pan_number, phone_number]
    );
